...
class V3BundlePublishAndDeltaTests(unittest.TestCase):
    ...
    def _create_fixture_db(path: Path) -> None:
        ...
        try:
            conn.executescript(
                ...
                VALUES
                  (1, '100', 'residential', '50', NULL, NULL, NULL, NULL, 90.0, 13.0000, 52.0000, 13.0010, 52.0010),
                  (2, '300', 'service', '20', NULL, NULL, NULL, NULL, 0.0, 13.0100, 52.0100, 13.0110, 52.0110);
    ...
    def test_build_v3_delta_pack_and_apply_patch(self):
        ...
        with sqlite3.connect(db_copy) as conn:
            ...
            rows = conn.execute("SELECT way_id, maxspeed FROM ways ORDER BY way_id").fetchall()
            row_map = {r[0]: r[1] for r in rows}
            self.assertEqual(row_map.get("100"), "30")
            self.assertEqual(row_map.get("200"), "20")
