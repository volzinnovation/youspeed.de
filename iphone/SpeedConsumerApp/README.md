# SpeedConsumerApp (v3-only)

Consumer iPhone app scaffold for on-road speed + speed-limit display.

## Design goals
- Keep the benchmark app (`SpeedDBBenchSketch`) stable for reproducibility.
- Use only v3 runtime data (`speeds_v3.sqlite`) for production consumer flow.
- Require a downloaded data bundle before driving mode.
- Support independent map updates via GitHub-hosted bundle manifest and optional SQL delta patches.

## Update flow
1. Discover manifests from bundled `BundleTargets.top10.json`, preferring Germany shard endpoints.
   - `YouSpeedV3ManifestURL` remains available as a build-time dev override.
2. Download and verify the selected map bundle; no map database is embedded in the app.
3. If delta path exists from the active version to a later target: download + verify + apply SQL patch.
4. Otherwise download the full `speeds_v3.sqlite` bundle.
   - If manifest provides `db_parts`, download parts and assemble local DB before activation.
5. Activate atomically and persist active bundle metadata.
6. If the active bundle is older than 30 days relative to the target version, skip deltas and force a full bundle reload.

Installed maps open offline using their saved metadata. Database integrity is
verified during download/update installation, never again during startup, local
recovery, coverage routing, or an already-current version check. Local access
checks only that the database file is available.

## Notes
- The app uses standard `SQLite3` and does not depend on runtime `mod_spatialite` loading.
- Delta patches are SQL scripts generated by `scripts/map/build_v3_delta_pack.py`.
- Speed lookup uses an RTree bbox prefilter with polyline distance refinement, optional heading-aware scoring, and continuity hysteresis for preferred way IDs.
- Release assets are public. The app does not accept or embed a repository credential.
- Manifest discovery comes from bundled targets by default; `YouSpeedV3ManifestURL` is only a build-time development override.

## Sources and license notices

The Info screen links to a searchable, offline Sources & credits list. The shared
`shared/attributions/sources.json` catalog supplies author credits, source and
license links, and modification notes for data, sign artwork, models, software,
and references. The full offline notices viewer includes both the shared notices
and the bundled model-pack notices. XcodeGen packages the `attributions` folder
directly; edit the shared source files rather than copies inside build products.
