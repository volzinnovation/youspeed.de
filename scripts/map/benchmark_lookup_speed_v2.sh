...
if [[ $# -lt 3 || $# -gt 8 ]]; then
  echo "Usage: $0 <dist_v2_dir> <lat> <lon> [heading=90] [repeats=10] [top_k=5] [polyline_top_n=250] [tile_radius=1]" >&2
  exit 1
fi
...
echo "Benchmarking v2 $dist_dir @ lat=$lat lon=$lon heading=$heading repeats=$repeats tile_radius=$tile_radius"
run_mode bbox
run_mode hybrid
run_mode polyline
