...
\begin{abstract}
This paper evaluates runtime data architectures for low-latency, offline-capable smartphone speed-limit inference using country-scale OpenStreetMap (OSM) data. We compare four storage/query scenarios across three query modes and a 30-day daily-diff update analysis using one common benchmark harness. Within this benchmark scope, a single-file embedded spatial database (S3) provides the strongest device-side latency and lower update overhead than the alternative embedded variant S4, while tiled packs (S2) improve substantially over the global-index baseline (S1). The result is an architecture recommendation for the current runtime-data layer, not a claim about end-to-end ISA effectiveness or safety impact.
\end{abstract}
...
\item A benchmark-scoped architecture recommendation balancing query latency and incremental update cost.
\end{enumerate}

\section{Related Work}
Map matching for transport applications has been widely studied, including geometric/topological methods \cite{quddus2007} and probabilistic sequence models such as HMM-based approaches \cite{newson2009,lou2009,goh2012,he2013}. In practice, these sequence models are the standard mechanism for stabilizing trajectory inference: they favor staying on the current road unless a turn transition is sufficiently plausible under network connectivity and motion evidence \cite{newson2009,hunter2014,goh2012,he2013}. Later work further improved robustness for sparse trajectories and route-level inference \cite{rahmani2013,quddus2015,jagadeesh2017,che2018}.

This model structure is also reflected in production routing engines. OSRM, Valhalla (Meili), and Mapbox map matching expose candidate emission/observation controls, transition plausibility settings (including GPS uncertainty or search radii), and confidence-style outputs for downstream use \cite{osrm_match_service,valhalla_meili,mapbox_map_matching_api}. To reduce online transition-check cost, precomputed transition structures have also been proposed; FMM uses a UBODT-style table so route-distance checks become lightweight lookups during matching \cite{yang2018fmm}.
...
When explicit speed tags are absent, the legal default baseline for Germany (\SI{50}{km/h} urban, \SI{100}{km/h} rural) is applied \cite{stvo_3}. This rule-aware fallback is necessary because most ways do not carry explicit speed tags. In the benchmark harness, the urban-versus-rural signal is approximated from built-up residential polygon containment with place-based fallback when no enclosing residential polygon is available; administrative polygons are retrieved separately for city labels and traceability. The inference order is: user input (if present), then way-level \texttt{maxspeed} retrieval through \texttt{bbox}/\texttt{hybrid}/\texttt{polyline}, and finally legal-rule fallback, which requires the area-context query measured here as \texttt{polycontainment}.
...
The benchmark uses a fixed Berlin probe point (\(52.5200,13.4050\)), heading \(90^\circ\), top-\(k=5\), and four measured query components (\texttt{bbox}, \texttt{hybrid}, \texttt{polyline}, \texttt{polycontainment}). The harness intentionally isolates storage/query architecture and therefore does not reproduce the full continuity-aware iPhone matcher; it measures candidate retrieval, geometric ranking, and area-context lookup under one common protocol.
...
For the benchmark harness's area-context retrieval, the measured built-up classification stage is \(3.14\) ms (S1), \(0.81\) ms (S2), \(0.74\) ms (S3), and \(2.79\) ms (S4). At the Berlin benchmark probe, all scenarios resolve the point as \emph{inside-city} under residential polygon containment; administrative polygon lookup remains a separate labeling step. The low millisecond-scale polycontainment latencies are expected because RTree prefiltering reduces candidate polygons to a very small set before exact ring containment checks.

To express combined way-query plus area-context latency as a simple query-throughput proxy under worst-case per-fix operation, the following relation is used:
\begin{equation}
f_{\text{cycle}}=\frac{1000}{t_{\text{maxspeed}}+t_{\text{polycontainment}}}, \qquad
v_{\text{proxy}}=3.6\cdot f_{\text{cycle}} \cdot \Delta s
\label{eq:speed}
\end{equation}
where \(t_{\text{maxspeed}}\) is way-query latency, \(t_{\text{polycontainment}}\) is polygon containment latency, and \(\Delta s=\SI{10}{m}\) is the inter-fix spacing assumption. These values are theoretical microbenchmark proxies, not operational vehicle-speed limits.

\begin{table}[!t]
\caption{Theoretical query-throughput proxy from worst-case per-fix combined latency (\(t_{\text{maxspeed}}+t_{\text{polycontainment}}\)).}
\label{tab:device_speed_envelope}
\centering
\begin{tabular}{lrrrr}
\toprule
Scenario & Mean ms & Hz & Best \(v_{\text{proxy}}\) & Worst \(v_{\text{proxy}}\) \\
\midrule
...
Under the present benchmark and its simple equal-weight treatment of latency and update proxies, S3 is the most practical default among the measured scenarios. The architecture provides low-latency on-device lookup, compact single-file deployment, and lower incremental update overhead than S4.
