...
The country registry and delivery-envelope schemas are additive. The DE pack is
the only production registry entry; FR/NL/BE have evaluation-only packs staged
in the app trees with completed offline export/parity evidence, but remain out
of the registry pending country action review, attached-device parity and
signing.
...
[`TSR_CH_PANORAMAX_READINESS.md`](../../docs/TSR_CH_PANORAMAX_READINESS.md).
CH is present only as an evaluation/shadow bundle after the first export-parity
pass; it remains absent from the runtime registry until the documented imagery,
artwork, legal, evaluation and attached-device gates pass.

Foreign evaluation packs are generated by
[`build_runtime_pack_metadata.py`](../../scripts/tsr/foreign-export/build_runtime_pack_metadata.py).
Their national catalogs use the shared `sign-pictograms/national/<CC>/` SVG/PNG
bytes and retain per-artwork Commons provenance, license, source URL and
hashes. Panoramax dataset archives are calibration/build inputs only and are
not runtime assets.
...
output parity, run the country-specific operational confidence benchmark under
the German field-calibration policy, and replay candidate
